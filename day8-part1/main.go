...
func main() {
	...
	for _, display := range displays {
		display.FindTrivialSignals()

		output, _ := display.DecodeOutput()
		decodedValueCount += len(output)
	}

	fmt.Println("total number of decoded output values", decodedValueCount)
}
