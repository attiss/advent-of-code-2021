...
func main() {
	...
	activeBoards := make([]Board, len(boards))
	copy(activeBoards, boards)
	for _, chosenNumber := range choseNumbers {
		var activeBoardsAfterRound []Board
		for _, board := range activeBoards {
			board.RemoveChosenNumber(chosenNumber)

			if board.IsWinningBoard() {
				if len(activeBoards) == 1 {
					fmt.Printf("last winner board: %#v\n", board)
					fmt.Printf("last winner board score: %d\n", board.GetScore())
			...
			activeBoardsAfterRound = append(activeBoardsAfterRound, board)
		}
		activeBoards = activeBoardsAfterRound
	}
}
